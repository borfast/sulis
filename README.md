...
`WithCSRFCookieName` validates the name with the same HTTP cookie-name rules as `WithCookieName`. A name without the `__Host-` prefix deliberately drops that browser-enforced origin-binding layer for the pure double-submit token; keep `RequireSameOrigin` and SameSite in place, and use this for a concrete local or integration requirement rather than production. `Secure`, `Path=/`, no `Domain`, and `HttpOnly=false` remain fixed. Renaming the cookie does not guarantee Secure-cookie acceptance over plain HTTP; use local TLS where the browser requires it.
