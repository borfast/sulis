...
// A value below 1 is rejected by NewService rather than quietly replaced by
// the default: a caller who asks for zero codes has asked for something this
// package cannot do, and silently handing back ten hid that.
func WithCount(n int) Option {
	return func(c *Config) { c.Count = n }
}
...
//
// It returns an error rather than a bare *Service so a nil store and an
// impossible code count are refused here, at construction, instead of
// surfacing later as a nil-pointer panic or a silently empty code set.
func NewService(store Store, opts ...Option) (*Service, error) {
	...
	switch {
	case store == nil:
		return nil, fmt.Errorf("recovery: store must not be nil")
	case cfg.Count <= 0:
		return nil, fmt.Errorf("recovery: count must be positive, got %d", cfg.Count)
	}
