...
| T205 | Last-factor deletion guard: new `Service.DeleteCredential(ctx, userID, id string, opts DeleteOptions) error` with `DeleteOptions{AllowLast bool}`, exactly as the brief's example — not a bare `force bool` parameter and not a separate `DeleteLastCredential` method | An options struct reads clearer at call sites (`DeleteOptions{AllowLast: true}` vs. a bare `true`) and leaves room for a future guard-related flag without another signature break. A separate method would mean two entry points enforcing the same invariant, which is more surface for the two to drift. `Store.DeleteCredential`/`DeleteCredentialsByUserID` themselves apply no guard — only `Service.DeleteCredential` does, after loading the user's credential list to both confirm ownership of `id` (returns `ErrPasskeyNotFound` if it isn't in that list) and count how many remain |
